 ...
 * On-curve points will be drawn in steelblue, off-curve points will be drawn in red.
 * @param  {CanvasRenderingContext2D} ctx - A 2D drawing context, like Canvas.
 * @param  {number} [x=0] - Horizontal position of the beginning of the text.
 * @param  {number} [y=0] - Vertical position of the *baseline* of the text.
 * @param  {number} [fontSize=72] - Font size in pixels. We scale the glyph units by `1 / unitsPerEm * fontSize`.
...
Glyph.prototype.drawPoints = function(ctx, x, y, fontSize) {
    ...
    var steelblueCircles = [];
    var redCircles = [];
    var path = this.path;
    for (var i = 0; i < path.commands.length; i += 1) {
        var cmd = path.commands[i];
        if (cmd.x !== undefined) {
            steelblueCircles.push({x: cmd.x, y: -cmd.y});
        }
    ...
    drawCircles(steelblueCircles, x, y, scale);
    ctx.fillStyle = 'red';
 ...
 * steelblue lines indicate the glyph bounding box.
