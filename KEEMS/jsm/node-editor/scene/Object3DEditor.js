...
export class Object3DEditor extends BaseNode {
	...
	_initTransform() {
		...
		const posZ = new NumberInput().setTagColor( 'steelblue' ).onChange( update );
		...
		const rotZ = new NumberInput().setTagColor( 'steelblue' ).setStep( rotationStep ).onChange( update );
		...
		const scaleZ = new NumberInput( 100 ).setTagColor( 'steelblue' ).setStep( rotationStep ).onChange( update );
